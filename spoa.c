...
enum spoa_state {
	SPOA_ST_CONNECTING = 0, SPOA_ST_PROCESSING, SPOA_ST_DISCONNECTING,
};

enum spoa_frame_type {
	SPOA_FRM_T_UNKNOWN = 0, SPOA_FRM_T_HAPROXY, SPOA_FRM_T_AGENT,
};
...
static const char *spoe_frm_err_reasons[SPOE_FRM_ERRS] = { [SPOE_FRM_ERR_NONE
		] = "normal", [SPOE_FRM_ERR_IO] = "I/O error", [SPOE_FRM_ERR_TOUT
		] = "a timeout occurred", [SPOE_FRM_ERR_TOO_BIG] = "frame is too big",
		[SPOE_FRM_ERR_INVALID] = "invalid frame received", [SPOE_FRM_ERR_NO_VSN
				] = "version value not found", [SPOE_FRM_ERR_NO_FRAME_SIZE
				] = "max-frame-size value not found", [SPOE_FRM_ERR_NO_CAP
				] = "capabilities value not found", [SPOE_FRM_ERR_BAD_VSN
				] = "unsupported version", [SPOE_FRM_ERR_BAD_FRAME_SIZE
		...
		[SPOE_FRM_ERR_RES] = "resource allocation error", [SPOE_FRM_ERR_UNKNOWN
...
static void check_buffer(struct spoe_frame *frame, struct chunk *chk, struct in_addr *ipv4) {
	...
	if ((int) chk->len > 0) {
		...
		if ((int) chk->len > 2 && (int) chk->ptr[0] == 0
				&& (int) chk->ptr[1] == 18) { // navis track = ok
			frame->ip_score = 50;
		}

		DEBUG(frame->worker,
				"Chb -> %02x %02x %02x %02x %02x %02x %02x " "%02x %02x %02x %02x %02x %02x %02x " "%02x %02x %02x %02x %02x %02x --> %s",
				...
				(unsigned char )chk->ptr[6], (unsigned char )chk->ptr[7],
...
static void check_ipv4_reputation(struct spoe_frame *frame,
	...
	DEBUG(frame->worker, "IP score for %.*s is %d", INET_ADDRSTRLEN, str,
			frame->ip_score);
}

static void check_ipv6_reputation(struct spoe_frame *frame,
...
static int check_proto_version(struct spoe_frame *frame, char **buf, char *end) {
	char *str, *p = *buf;
	uint64_t sz;
	int ret;
	...
	DEBUG(frame->worker, "<%lu> Supported versions : %.*s", frame->client->id,
			(int )sz, str);
 ...
 * read bytes otherwise. */
static int check_max_frame_size(struct spoe_frame *frame, char **buf, char *end) {
	char *p = *buf;
	uint64_t sz;
	int type, ret;
 ...
 * read bytes otherwise. */
static int check_healthcheck(struct spoe_frame *frame, char **buf, char *end) {
	char *p = *buf;
	int type, ret;
	...
	DEBUG(frame->worker, "<%lu> HELLO healthcheck : %s", frame->client->id,
			(frame->hcheck ? "true" : "false"));
 ...
 * read bytes otherwise. */
static int check_capabilities(struct spoe_frame *frame, char **buf, char *end) {
	...
	DEBUG(frame->worker, "<%lu> HAProxy capabilities : %.*s", client->id,
			(int )sz, str);
	...
	while (sz) {
		...
		if (sz >= 10 && !strncmp(str, "pipelining", 10)) {
			...
			if (!sz || isspace(*str) || *str == ',') {
				DEBUG(frame->worker, "<%lu> HAProxy supports frame pipelining",
						client->id);
			...
			sz -= 13;
			if (!sz || isspace(*str) || *str == ',') {
				DEBUG(frame->worker, "<%lu> HAProxy supports fragmented frame",
						client->id);
 ...
 * read bytes otherwise. */
static int check_engine_id(struct spoe_frame *frame, char **buf, char *end) {
	struct client *client = frame->client;
	char *str, *p = *buf;
	uint64_t sz;
	int ret;
	...
	DEBUG(frame->worker, "<%lu> HAProxy engine id : %.*s", client->id, (int )sz,
			str);
...
static int acc_payload(struct spoe_frame *frame) {
	struct client *client = frame->client;
	char *buf;
...
static int check_discon_status_code(struct spoe_frame *frame, char **buf,
		char *end) {
	char *p = *buf;
	uint64_t sz;
	int type, ret;
	...
	DEBUG(frame->worker, "<%lu> Disconnect status code : %u", frame->client->id,
			frame->client->status_code);
 ...
 * of read bytes otherwise. */
static int check_discon_message(struct spoe_frame *frame, char **buf, char *end) {
	char *str, *p = *buf;
	uint64_t sz;
	int ret;
	...
	DEBUG(frame->worker, "<%lu> Disconnect message : %.*s", frame->client->id,
			(int )sz, str);
...
static int handle_hahello(struct spoe_frame *frame) {
	struct client *client = frame->client;
	char *p, *end;
	...
	while (p < end) {
		...
		/* Check "engine-id" K/V item */
			...
			DEBUG(frame->worker, "<%lu> Skip K/V item : key=%.*s", client->id,
					(int )sz, str);
...
static int handle_hadiscon(struct spoe_frame *frame) {
	struct client *client = frame->client;
	char *p, *end;
 ...
 * bytes. */
static int handle_hanotify(struct spoe_frame *frame) {
	struct client *client = frame->client;
	char *p, *end;
	...
	DEBUG(frame->worker,
			"<%lu> STREAM-ID=%u - FRAME-ID=%u" " - %s frame received" " - frag_len=%u - len=%u - offset=%ld",
 ...
 * of read bytes. */
static int handle_hafrag(struct spoe_frame *frame) {
	struct client *client = frame->client;
	char *p, *end;
	uint64_t stream_id, frame_id;
	...
	DEBUG(frame->worker, "<%lu> Decode Next part of a fragmented frame",
			client->id);
	...
	if (frame->flags & SPOE_FRM_FL_ABRT) {
		DEBUG(frame->worker,
				"<%lu> STREAM-ID=%u - FRAME-ID=%u" " - Abort processing of a fragmented frame" " - frag_len=%u - len=%u - offset=%ld",
				client->id, frame->stream_id, frame->frame_id, frame->frag_len,
				frame->len, p - frame->buf);
		goto ignore;
	}

	DEBUG(frame->worker,
			"<%lu> STREAM-ID=%u - FRAME-ID=%u" " - %s fragment of a fragmented frame received" " - frag_len=%u - len=%u - offset=%ld",
			client->id, frame->stream_id, frame->frame_id,
			(frame->flags & SPOE_FRM_FL_FIN) ? "last" : "next", frame->frag_len,
			frame->len, p - frame->buf);
 ...
 * bytes. */
static int prepare_agenthello(struct spoe_frame *frame) {
	struct client *client = frame->client;
	char *p, *end;
	...
	*p++ = SPOE_DATA_T_STR;
	...
	DEBUG(frame->worker, "<%lu> Agent version : %s", client->id, SPOP_VERSION);
	...
	*p++ = SPOE_DATA_T_UINT32;
	...
	DEBUG(frame->worker, "<%lu> Agent maximum frame size : %u", client->id,
	...
	*p++ = SPOE_DATA_T_STR;
	...
	DEBUG(frame->worker, "<%lu> Agent capabilities : %.*s", client->id, n,
			capabilities);
 ...
 * written bytes. */
static int prepare_agentdicon(struct spoe_frame *frame) {
	struct client *client = frame->client;
	char *p, *end;
	const char *reason;
	int rlen;
	...
	*p++ = SPOE_DATA_T_UINT32;
	...
	DEBUG(frame->worker, "<%lu> Disconnect status code : %u", client->id,
			client->status_code);
	...
	*p++ = SPOE_DATA_T_STR;
	...
	DEBUG(frame->worker, "<%lu> Disconnect message : %s", client->id, reason);
 ...
 * bytes. */
static int prepare_agentack(struct spoe_frame *frame) {
	char *p, *end;
	unsigned int flags = SPOE_FRM_FL_FIN;
	...
	*p++ = SPOE_FRM_T_AGENT_ACK;
	...
	DEBUG(frame->worker, "STREAM-ID=%u - FRAME-ID=%u", frame->stream_id,
			frame->frame_id);
...
static void release_client(struct client *c) {
...
static void worker_monitor_cb(evutil_socket_t fd, short events, void *arg) {
	struct worker *worker = arg;

	LOG(worker, "%u clients connected (%u frames)", worker->nbclients,
			worker->nbframes);
}

static void process_frame_cb(evutil_socket_t fd, short events, void *arg) {
...
static void read_frame_cb(evutil_socket_t fd, short events, void *arg) {
	...
	DEBUG(client->worker, "<%lu> New Frame of %u bytes received", client->id,
			frame->len);
	...
	case SPOA_ST_DISCONNECTING:
		disconnecting: if (handle_hadiscon(frame) < 0) {
			LOG(client->worker, "Failed to decode DISCONNECT frame");
			goto disconnect;
		}
		if (client->status_code != SPOE_FRM_ERR_NONE)
			LOG(client->worker, "<%lu> Peer closed connection: %s", client->id,
					spoe_frm_err_reasons[client->status_code]);
		goto disconnect;
	}
	...
	ignore_frame: reset_frame(frame);
	return;

	process_frame: process_incoming_frame(frame);
	client->incoming_frame = NULL;
	return;

	write_frame: write_frame(client, frame);
	client->incoming_frame = NULL;
	return;

	disconnect: client->state = SPOA_ST_DISCONNECTING;
	...
	close: release_client(client);
}

static void write_frame_cb(evutil_socket_t fd, short events, void *arg) {
	struct client *client = arg;
	struct spoe_frame *frame;
	int n;
	...
	DEBUG(client->worker, "<%lu> Frame of %u bytes sent", client->id,
			frame->len);

	switch (client->state) {
	case SPOA_ST_CONNECTING:
		if (frame->hcheck == true) {
			DEBUG(client->worker, "<%lu> Close client after healthcheck",
					client->id);
			goto close;
		}
...
static void accept_cb(int listener, short event, void *arg) {
	struct worker *worker;
	struct client *client;
	int fd;
...
static int parse_processing_delay(const char *str) {
	unsigned long value;
...
int main(int argc, char **argv) {
	...
	for (i = 0; i < num_workers; ++i) {
		...
		w->monitor_event = event_new(w->base, fd, EV_PERSIST, worker_monitor_cb,
				(void*) w);
		if (w->monitor_event == NULL
				|| event_add(w->monitor_event, (struct timeval[] ) { { 5, 0 } })
						< 0) {
			LOG(&null_worker, "Failed to create monitor event for worker %02d",
					w->id);
			goto error;
		}

		if (pthread_create(&w->thread, NULL, worker_function, (void*) w)) {
			LOG(&null_worker, "Failed to start thread for worker %02d : %m",
					w->id);
		}
	...
	DEBUG(&null_worker,
			"Server is ready" " [fragmentation=%s - pipelining=%s - async=%s - debug=%s - max-frame-size=%u]",
			(fragmentation ? "true" : "false"), (pipelining ? "true" : "false"),
			(async ? "true" : "false"), (debug ? "true" : "false"),
			max_frame_size);
